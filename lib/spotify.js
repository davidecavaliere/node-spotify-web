 ...
 *
 * Instead this makes a request to an API which runs the Flash inside a browser.
 * See https://github.com/1994rstefan/Spotify-Ping-Pong-API
 *
 * @param {String} ping the argument sent from the request
 */
Spotify.prototype.sendPong = function(ping) {
  var encodedPing = ping.replace(/ /g, '-');
  var that = this;

  http.get('http://ping-pong.spotify.nodestuff.net/'+ encodedPing, function (res) {
    var resData = ''
    res.on('data', function (data) {
      resData += data;
    });
    res.on('end', function () {
      var decodedRes = JSON.parse(resData);
      if(decodedRes.status !== 100){
        that.emit(
          'error',
          new Error('Unable to receive flash pong, error code was '+ decodedRes.status +' (see https://github.com/1994rstefan/Spotify-Ping-Pong-API#status-codes)')
        );
        return;
      }
      var pong = decodedRes.pong.replace(/-/g, ' ');
      debug('received flash ping %j, sending pong: %j', ping, pong);
      that.sendCommand('sp/pong_flash2', [pong]);
    });
  }).on('error', function (e) {
      that.emit('error', e);
  });
